...
type kubernetesRestorer struct {
	...
	resticRestorerFactory      uploader.RestorerFactory
...
func (kr *kubernetesRestorer) RestoreWithResolvers(
	...
	var resticRestorer uploader.Restorer
...
type restoreContext struct {
	...
	volumeSnapshotterGetter        VolumeSnapshotterGetter
	resticRestorer                 uploader.Restorer
...
func (ctx *restoreContext) restoreItem(obj *unstructured.Unstructured, groupResource schema.GroupResource, namespace string) (Result, Result) {
	...
	if groupResource == kuberesource.Pods {
		...
		if ctx.resourceIncludesExcludes.ShouldInclude(kuberesource.PersistentVolumeClaims.String()) &&
			...
			len(uploader.GetVolumeBackupsForPod(ctx.podVolumeBackups, pod, originalNamespace)) > 0 {
...
func hasResticBackup(unstructuredPV *unstructured.Unstructured, ctx *restoreContext) bool {
	...
	for _, pvb := range ctx.podVolumeBackups {
		if pvb.Spec.Pod.Namespace == pv.Spec.ClaimRef.Namespace && pvb.GetAnnotations()[uploader.PVCNameAnnotation] == pv.Spec.ClaimRef.Name {
			found = true
			break
		}
	}
