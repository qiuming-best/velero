...
// Command represents a repository command.
type Command struct {
	Command               string
...
func (c *Command) StringSlice() []string {
	...
	if c.InsecureSkipTSLVerify {
		res = append(res, insecureSkipTLSVerifyFlag())
	}

	// If VELERO_SCRATCH_DIR is defined, put the repository cache within it. If not,
	// allow repository to choose the location. This makes running either in-cluster
	// or local (dev) work properly.
	if scratch := os.Getenv("VELERO_SCRATCH_DIR"); scratch != "" {
		res = append(res, cacheDirFlag(filepath.Join(scratch, ".cache", "repository")))
	}
...
func insecureSkipTLSVerifyFlag() string {
	return "--insecure-tls=true"
}
