...
// repositoryEnsurer ensures that Velero repositories are created and ready.
...
func (r *repositoryEnsurer) EnsureRepo(ctx context.Context, namespace, volumeNamespace, backupLocation string) (*velerov1api.ResticRepository, error) {
	...
	// running this for the same inputs, both might find no UnifiedRepository exists, then
	...
	// each repository, and we just tried to create it, checked for an
	...
	if len(repos) == 1 {
		if repos[0].Status.Phase != velerov1api.ResticRepositoryPhaseReady {
			return nil, errors.Errorf("Repository %s is not ready: %s, phase %s", repos[0].Name, repos[0].Status.Message, repos[0].Status.Phase)
		}
	...
	if _, err := r.repoClient.ResticRepositories(namespace).Create(context.TODO(), repo, metav1.CreateOptions{}); err != nil {
		return nil, errors.Wrapf(err, "unable to create repository resource")
	}
	...
	case <-time.After(time.Minute):
		return nil, errors.New("timed out waiting for repository to become ready")
	case <-ctx.Done():
		return nil, errors.New("timed out waiting for repository to become ready")
	case res := <-repoChan:
		if res.Status.Phase == velerov1api.ResticRepositoryPhaseNotReady {
			return nil, errors.Errorf("Repository is not ready: %s", res.Status.Message)
		}
