...
func RepoKeySelector() *corev1api.SecretKeySelector {
	// For now, all repos share the same key so we don't need the repoName to fetch it.
	// When we move to full-backup encryption, we'll likely have a separate key per repo
