...
// reposiotry commands, i.e. everything except the "/<repo-name>".
func getRepoPrefix(location *velerov1api.BackupStorageLocation, legacyRepo bool) (string, error) {
	...
	if location.Spec.ObjectStorage != nil {
		...
		prefix = layout.GetRepositoryDir(legacyRepo)
	}

	backendType := GetBackendType(location.Spec.Provider)

	if repoPrefix := location.Spec.Config["RepoPrefix"]; repoPrefix != "" {
	...
	return "", errors.New("Repository prefix (RepoPrefix) not specified in backup storage location's config")
}
...
func GetRepoIdentifier(location *velerov1api.BackupStorageLocation, name string, legacyRepo bool) (string, error) {
	prefix, err := getRepoPrefix(location, legacyRepo)
	if err != nil {
		return "", err
	}
